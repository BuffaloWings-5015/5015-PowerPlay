...
public class PolePipeline extends OpenCvPipeline {
    ...
    public xposser xpoossser = xposser.CENTER;
    ...
    public Mat processFrame(Mat input) {
        ...
        if (Core.countNonZero(yelMat) > 0) {
            ...
            Imgproc.rectangle(input, Rect.tl(), Rect.br(), new Scalar(0, 0, 255), 1);
            position = Rect.tl();
            String text = Rect.tl().toString();
            Imgproc.putText(input, text, position, font, scale, new Scalar(0, 0, 255), thickness);
            xPos = Rect.x;
        }
...
*/
    ...
    public double polePos() {
        return xPos;
    }

    public xposser polePosEnum() {
        return xpoossser;
